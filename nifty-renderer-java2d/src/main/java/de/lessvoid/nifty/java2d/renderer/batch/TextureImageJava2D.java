...
/**
 * Implementation of TextureJava2D for single-image (non-atlas) textures. TextureImageJava2D writes image data to a
 * {@link java.awt.image.BufferedImage} that can be hardware accelerated depending on the acceleration priority set.
 * BufferedImage is backed internally in Java2D by OpenGL/D3D hardware (if accelerated) textures but handles render-to
 * operations in software, so it is best used in this case for textures that are non-atlas'd but will be read from in
 * future hardware draw operations.
 *
...
class TextureImageJava2D implements TextureJava2D {
   ...
   * @param height
   * @param priority
   *          0 < priority < 1 where 0 is never hardware accelerated and 1 is max priority
   */
  TextureImageJava2D(@Nonnull final ImageFactory imageFactory, final int width, final int height, final float priority, final boolean flipYAxis) {
    this.id = idTick++;
    this.width = width;
    this.height = height;
