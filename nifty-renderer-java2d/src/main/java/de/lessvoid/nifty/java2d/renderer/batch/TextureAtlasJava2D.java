...
/**
 * Implementation of TextureJava2D for atlas textures. TextureAtlasJava2D writes image data to a
 * {@link java.awt.image.VolatileImage} that is fully hardware accelerated (unless priority is set to 0 or system VRAM
 * is full). The backing VolatileImage is hardware accelerated both in render-to and render-from operations, so
 * TextureAtlasJava2D should provide the maximum possible performance for storing multiple sub-images in one large
 * texture object. VolatileImage data, however, may become invalid or be flushed from video memory at any given time; in
 * order to prevent loss of atlas data, TextureAtlasJava2D creates and stores a {@link java.awt.image.BufferedImage}
 * snapshot of the atlas after each write operation. This snapshot is then used to restore the contents of the
 * VolatileImage in the case that its data is lost or invalidated.
 *
...
class TextureAtlasJava2D implements TextureJava2D {
   ...
   * @param priority
   *          acceleration priority value
   */
  TextureAtlasJava2D(@Nonnull final ImageFactory imageFactory, final int width, final int height, final float priority, final boolean flipYAxis) {
  ...
  public void writeImageToTexture(@Nonnull final Image img, final int x, final int y) {
    Java2DImage j2dImage = (Java2DImage) imageFactory.create(imageFactory.asByteBuffer(img),
                                                             img.getWidth(),
                                                             img.getHeight());
    if (texVI == null) {
      initTextureVI(width, height);
  ...
  @Override
  public void drawTexture(@Nonnull final Graphics2D drawGraphics,
                          final int destX,
                          final int destY,
                          final int destWidth,
  ...
  public void setAccelerationPriority(final float priority) {
    if (priority >= 0 && priority <= 1) this.priority = priority;
  }

  @Override
  public boolean invertsYAxis() {
  ...
  /*
   * We use a java.util.concurrent.ExecutorService to handle updating the BufferedImage snapshot. This saves the
   * renderer thread from having to consume valuable frame time to update the snapshot; there should not be any
   * synchronization issues since getSnapshot() is a read-only operation. A thread pool of size n=1 should be used to
   * ensure that update requests are executed asynchronously.
   */
